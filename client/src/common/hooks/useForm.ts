...
enum ActionType {
  ...
  SET_FIELD_UNTOUCHED = "SET_FIELD_UNTOUCHED",
...
const reducer = (state: FormState, action: Action) => {
  switch (action.type) {
    ...
    case ActionType.SET_FIELD_UNTOUCHED:
      return {
        ...state,
        touched: {
          ...state.touched,
...
interface UseFormProps {
  initialValues: { [key: string]: string | boolean };
  onAnyChangeValidator: (values: object) => Promise<object>;
}
...
export const useForm = ({ initialValues, onAnyChangeValidator }: UseFormProps) => {
  ...
  const setFieldTouched = (fieldName: string) => dispatch({ type: ActionType.SET_FIELD_TOUCHED, key: fieldName });

  const setFieldUntouched = (fieldName: string) => dispatch({ type: ActionType.SET_FIELD_UNTOUCHED, key: fieldName });

  const setFieldsTouched = () => dispatch({ type: ActionType.SET_FIELDS_TOUCHED });
